...
int is_task_sleeping(const char *path)
{
    ...
    while (fgets(bufferline, sizeof(bufferline), status_file) != NULL) {
        if (strncmp(bufferline, "State:", 6) == 0) {
            fclose(status_file);
            return (strchr(bufferline, 'S') != NULL || strchr(bufferline,
                'D') != NULL || strchr(bufferline, 'I') != NULL) ? 1 : 0;
        }
    }
    fclose(status_file);
    return 0;
...
int get_cpu_task_sleeping(void)
{
    ...
    while (entry != NULL) {
        if (isdigit(entry->d_name[0])) {
            ...
            sleeping_task += is_task_sleeping(path);
        }
        entry = readdir(proc_dir);
    }
    closedir(proc_dir);
    printw("%d sleeping, ", sleeping_task);
}
